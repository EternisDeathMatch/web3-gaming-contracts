...
describe("CollectionFactory", function () {
  ...
  beforeEach(async function () {
    ...
    factory = (await Factory.deploy(initialFee, ownerAddress)) as CollectionFactory;
  ...
  it("deploys a new collection when fee is paid", async function () {
    const tx = factory.connect(await ethers.getSigner(userAddress)).deployCollection(
      "Game",
      "GM",
      "Desc",
      "uri/",
    ...
    await expect(tx)
      ...
      .withArgs(anyValue, userAddress, "Game", "GM", 10n);
  ...
  it("reverts if insufficient fee", async function () {
    await expect(
      factory.connect(await ethers.getSigner(userAddress)).deployCollection(
        "G",
        "G",
        "D",
        "u/",
  ...
  it("only owner can update settings", async function () {
    await expect(
      factory.connect(await ethers.getSigner(userAddress)).updateDeploymentFee(initialFee)
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("reverts withdraw when no fees accumulated", async function () {
    await expect(factory.withdrawFees()).to.be.revertedWith("No fees to withdraw");
  });

  it("correctly reports deployed vs non-deployed collections", async function () {
    // deploy one collection
    await factory.connect(await ethers.getSigner(userAddress)).deployCollection(
      "X",
      "X",
      "Desc",
      "uri/",
  ...
  it("tracks total collections count properly", async function () {
    ...
    await factory.connect(await ethers.getSigner(userAddress)).deployCollection(
      "A","A","Desc","uri/",1,userAddress,0,{ value: initialFee }
    );
    expect(await factory.getTotalCollections()).to.equal(1n);
    await factory.connect(await ethers.getSigner(userAddress)).deployCollection(
      "B","B","Desc","uri/",1,userAddress,0,{ value: initialFee }
    );
