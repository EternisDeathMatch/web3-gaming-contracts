...
// If your build outputs IncentiveEngine.json, this import will work.
...
export async function getEngineImplAddress(proxyAddr: string, out: HTMLElement) {
  try {
...
export async function callEngineFn({
  proxyAddr,
  fnSig,
  fnArgs,
  isWrite,
  ...
  proxyAddr: string;
  fnSig: string;   // e.g. "setSplit(bytes32,(uint16,uint16,uint16,address,address,bool,bool,bool))"
  fnArgs: any[];   // e.g. ["0xscope...", {...split...}]
  isWrite: boolean;
  ...
  try {
    ...
    let abi = (IncentiveEngineArtifact as any)?.abi ?? [];
    let useAbi = false;
    try {
